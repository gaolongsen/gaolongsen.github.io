...
In the article "Robot Positive Kinematics - Understanding Transformation Matrices", we introduced the rotation matrix <img src="https://latex.codecogs.com/svg.image?_{}^{A}\textrm{R}_B" title="_{}^{A}\textrm{R}_B" /> (pose relationship between {A} and {B}) representing the projection of the unit vectors of the three axes of the coordinate system {B} in the coordinate system {A} (i.e. unit vectors in the three axes of coordinates {B} in the coordinate {A}), and in the article "Robot positive kinematics - homogenous transformation matrices", we also saw that <img src="https://latex.codecogs.com/svg.image?p_{B_{org}}^{A}&space;" title="p_{B_{org}}^{A} " />(position relationship between {A} and {B}) represents the coordinates of the origin of the coordinate system {B} in the coordinate system {A}.