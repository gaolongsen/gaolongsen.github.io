...
So far we have the pose $ ^AR_B^T $ of the coordinate system {A} with respect to the coordinate system {B}, and the coordinates of the origin of the coordinate system {A} described in the coordinate system {B} $ p_{A_{org}}^{B} = -^AR_B^T \cdot p_{B_{org}}^A $. Thus the inverse of the transformation matrix is given by
